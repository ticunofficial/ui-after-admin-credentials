$(document).ready(function () {
    ...
    let tbl = $('#users_table').DataTable({
        ...
        columns: [
            {
                data: function (row) {
                    return `<div class="d-flex align-items-center"> <div class="symbol symbol-circle symbol-50px overflow-hidden mr-3">  <div> <img src="${row.photo_url}" alt="User Image" class="user-avatar-img"> </div> </div> <div class="d-flex flex-column"> <a href="javascript:void(0)" class="mb-1 user-name-data">${htmlSpecialCharsDecode(row.name)}</a> <span class="user-email-data">${row.email}</span> </div> </div> `
                }, name: 'name',
            },
            {
                data: function (data) {
            ...
            {
                data: function (data) {
                    return (data.privacy) ? '<span class="public-badge py-1 px-2">' + Lang.get('messages.group.public') + '</span>' : '<span class="private-badge py-1 px-2">' + Lang.get('messages.group.private') +'</span>';
                },
                name: 'privacy',
                'searchable': false,
            },
        ...
        $.ajax({
            ...
            success: function (obj) {
                ...
                displayToastr(Lang.get('messages.new_keys.success'), 'success',
                    header + Lang.get('messages.new_keys.has_been_restored'));
            },
            error: function (data) {
                displayToastr(Lang.get('messages.new_keys.error'), 'error', data.responseJSON.message);
            },
    ...
    window.validatePasswordConfirmation = function () {
        ...
        if (passwordConfirmation === '') {
            displayToastr(Lang.get('messages.new_keys.error'), 'error',
                (Lang.get('messages.new_keys.password_confirmation_required')));
            return false;
        }
        return true;
    };
    ...
    window.validateMatchPasswords = function () {
        ...
        if (passwordConfirmation !== password) {
            displayToastr(Lang.get('messages.new_keys.error'), 'error',
                (Lang.get('messages.new_keys.password_and_password_confirmation_did_not_match')));
            return false;
        }
        return true;
    };
